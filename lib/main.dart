...
void main() {
  ...
  runApp(const ProviderScope(child: MTCApp()));
}
...
class _InitialFlowState extends State<InitialFlow> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_isSynced) {
      return IdentitySyncScreen(onSyncComplete: () => setState(() => _isSynced = true));
    }
...
class _MainNavigationState extends ConsumerState<MainNavigation> {
        ...
        children: [
          DrawerHeader(
            decoration: const BoxDecoration(
              gradient: LinearGradient(colors: [MTCTheme.primaryBlue, MTCTheme.primaryNavy]),
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisAlignment: MainAxisAlignment.end,
              children: [
                const Icon(Icons.shield_rounded, color: MTCTheme.accentTeal, size: 40),
                const SizedBox(height: 10),
                Text('CallShield Modules', style: GoogleFonts.outfit(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          _drawerItem(context, 'Elite Premium', Icons.star_rounded, const PremiumStatusScreen(), color: MTCTheme.accentTeal),
          _drawerItem(context, 'Report a Threat', Icons.report_rounded, const ReportFormScreen()),
          _drawerItem(context, 'Call Simulator', Icons.phone_in_talk_rounded, const SimulationScreen()),
          _drawerItem(context, 'Native OS Shield', Icons.security_rounded, const NativeShieldScreen()),
          _drawerItem(context, 'Global Analytics', Icons.analytics_rounded, const AnalyticsScreen()),
          _drawerItem(context, 'Blocked Calls', Icons.block_flipped, const BlockedListScreen()),
          _drawerItem(context, 'My Reports', Icons.list_alt_rounded, const ReportsScreen()),
          _drawerItem(context, 'Live Alerts', Icons.notification_important_rounded, const AlertsScreen()),
  ...
  Widget _drawerItem(BuildContext context, String title, IconData icon, Widget screen, {Color? color}) {
    return ListTile(
      leading: Icon(icon, color: color ?? Colors.white70),
      title: Text(title, style: GoogleFonts.outfit(color: color ?? Colors.white, fontWeight: FontWeight.bold)),
      onTap: () {
